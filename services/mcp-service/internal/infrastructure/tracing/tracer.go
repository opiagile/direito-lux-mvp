...
import (
	...
	appconfig "github.com/direito-lux/mcp-service/internal/infrastructure/config"
...
// MCP-specific tracing functions

// TraceMCPSession cria span para sessão MCP
func TraceMCPSession(ctx context.Context, sessionID, channel string) (opentracing.Span, context.Context) {
	span, ctx := StartSpanFromContext(ctx, fmt.Sprintf("MCP_SESSION %s", channel))
	
	span.SetTag("mcp.session_id", sessionID)
	span.SetTag("mcp.channel", channel)
	ext.Component.Set(span, "mcp-session")
	
	return span, ctx
}

// TraceMCPTool cria span para execução de ferramenta MCP
func TraceMCPTool(ctx context.Context, toolName, channel string) (opentracing.Span, context.Context) {
	span, ctx := StartSpanFromContext(ctx, fmt.Sprintf("MCP_TOOL %s", toolName))
	
	span.SetTag("mcp.tool_name", toolName)
	span.SetTag("mcp.channel", channel)
	ext.Component.Set(span, "mcp-tool")
	
	return span, ctx
}

// TraceMCPConversation cria span para conversa MCP
func TraceMCPConversation(ctx context.Context, conversationID, channel string) (opentracing.Span, context.Context) {
	span, ctx := StartSpanFromContext(ctx, fmt.Sprintf("MCP_CONVERSATION %s", channel))
	
	span.SetTag("mcp.conversation_id", conversationID)
	span.SetTag("mcp.channel", channel)
	ext.Component.Set(span, "mcp-conversation")
	
	return span, ctx
}

// TraceClaudeAPI cria span para chamada da Claude API
func TraceClaudeAPI(ctx context.Context, model, operation string) (opentracing.Span, context.Context) {
	span, ctx := StartSpanFromContext(ctx, fmt.Sprintf("CLAUDE_API %s", operation))
	
	span.SetTag("claude.model", model)
	span.SetTag("claude.operation", operation)
	ext.Component.Set(span, "claude-api")
	ext.SpanKind.Set(span, ext.SpanKindRPCClientEnum)
	
	return span, ctx
}

// TraceBotInteraction cria span para interação com bot
func TraceBotInteraction(ctx context.Context, botType, interactionType string) (opentracing.Span, context.Context) {
	span, ctx := StartSpanFromContext(ctx, fmt.Sprintf("BOT %s %s", botType, interactionType))
	
	span.SetTag("bot.type", botType)
	span.SetTag("bot.interaction_type", interactionType)
	ext.Component.Set(span, "bot-interaction")
...
// SetSpanMCPContext adiciona contexto MCP ao span
func SetSpanMCPContext(span opentracing.Span, sessionID, channel, toolName string) {
	if span != nil {
		if sessionID != "" {
			span.SetTag("mcp.session_id", sessionID)
		}
...
// TracedMCPOperation executa operação MCP com tracing automático
func TracedMCPOperation(ctx context.Context, operationName, sessionID, channel, toolName string, fn func(context.Context) error) error {
	span, ctx := StartSpanFromContext(ctx, operationName)
	defer span.Finish()

	// Adicionar contexto MCP
	SetSpanMCPContext(span, sessionID, channel, toolName)
