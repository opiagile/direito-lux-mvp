...
type Metrics struct {
	...
	MCPSessions           *prometheus.GaugeVec
	MCPToolExecutions     *prometheus.CounterVec
	MCPToolDuration       *prometheus.HistogramVec
	MCPConversations      *prometheus.CounterVec
	MCPQuotaUsage         *prometheus.GaugeVec
	MCPClaudeAPI          *prometheus.CounterVec
	MCPBotInteractions    *prometheus.CounterVec
	MCPChannelActivity    *prometheus.CounterVec
...
func NewMetrics(cfg *config.Config, logger *zap.Logger) (*Metrics, error) {
	...
	m := &Metrics{
		...
		// MCP Specific Metrics
		MCPSessions: prometheus.NewGaugeVec(
			prometheus.GaugeOpts{
				Namespace: namespace,
				Name:      "mcp_sessions_active",
				Help:      "Number of active MCP sessions",
			},
			[]string{"channel", "tenant_id"}, // whatsapp, telegram, slack, web
		),
		...
		MCPToolExecutions: prometheus.NewCounterVec(
			...
			[]string{"tool_name", "channel", "tenant_id", "status"},
		),
		...
		MCPToolDuration: prometheus.NewHistogramVec(
			...
			[]string{"tool_name", "channel", "tenant_id"},
		),

		MCPConversations: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Namespace: namespace,
				Name:      "mcp_conversations_total",
				Help:      "Total number of MCP conversations",
			},
			[]string{"channel", "tenant_id", "status"}, // initiated, completed, failed
		),

		MCPQuotaUsage: prometheus.NewGaugeVec(
			prometheus.GaugeOpts{
				Namespace: namespace,
				Name:      "mcp_quota_usage_percent",
				Help:      "MCP quota usage percentage by tenant",
			},
			[]string{"tenant_id", "quota_type"}, // tokens, requests, tools
		),

		MCPClaudeAPI: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Namespace: namespace,
				Name:      "mcp_claude_api_requests_total",
				Help:      "Total number of Claude API requests",
			},
			[]string{"model", "tenant_id", "status"},
		),

		MCPBotInteractions: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Namespace: namespace,
				Name:      "mcp_bot_interactions_total",
				Help:      "Total number of bot interactions",
			},
			[]string{"bot_type", "interaction_type", "tenant_id", "status"},
		),

		MCPChannelActivity: prometheus.NewCounterVec(
			prometheus.CounterOpts{
				Namespace: namespace,
				Name:      "mcp_channel_activity_total",
				Help:      "Total channel activity events",
			},
			[]string{"channel", "activity_type", "tenant_id"},
		),
...
func (m *Metrics) register() error {
	collectors := []prometheus.Collector{
		...
		m.MCPQuotaUsage,
		m.MCPClaudeAPI,
		m.MCPBotInteractions,
		m.MCPChannelActivity,
		m.TenantOperations,
		m.UserOperations,
		m.ProcessOperations,
		m.EventsPublished,
...
// RecordEvent registra métricas de eventos
func (m *Metrics) RecordEvent(eventType, operation string, duration time.Duration, err error) {
	if !m.config.Metrics.Enabled {
		return
	}
...
// MCP-specific metric recording functions

// RecordMCPSession registra sessão MCP ativa
func (m *Metrics) RecordMCPSession(channel, tenantID string, active bool) {
	if !m.config.Metrics.Enabled {
		return
	}

	if active {
		m.MCPSessions.WithLabelValues(channel, tenantID).Inc()
	} else {
		m.MCPSessions.WithLabelValues(channel, tenantID).Dec()
	}
}

// RecordMCPToolExecution registra execução de ferramenta MCP
func (m *Metrics) RecordMCPToolExecution(toolName, channel, tenantID string, duration time.Duration, err error) {
	if !m.config.Metrics.Enabled {
		return
	}
	...
	m.MCPToolExecutions.WithLabelValues(toolName, channel, tenantID, status).Inc()
	m.MCPToolDuration.WithLabelValues(toolName, channel, tenantID).Observe(duration.Seconds())
}

// RecordMCPConversation registra conversa MCP
func (m *Metrics) RecordMCPConversation(channel, tenantID, status string) {
	if !m.config.Metrics.Enabled {
		return
	}

	m.MCPConversations.WithLabelValues(channel, tenantID, status).Inc()
}

// UpdateMCPQuotaUsage atualiza uso de quota MCP
func (m *Metrics) UpdateMCPQuotaUsage(tenantID, quotaType string, usagePercent float64) {
	if !m.config.Metrics.Enabled {
		return
	}

	m.MCPQuotaUsage.WithLabelValues(tenantID, quotaType).Set(usagePercent)
}

// RecordMCPClaudeAPICall registra chamada para Claude API
func (m *Metrics) RecordMCPClaudeAPICall(model, tenantID string, success bool) {
	if !m.config.Metrics.Enabled {
		return
	}
	...
	m.MCPClaudeAPI.WithLabelValues(model, tenantID, status).Inc()
}

// RecordMCPBotInteraction registra interação com bot
func (m *Metrics) RecordMCPBotInteraction(botType, interactionType, tenantID string, success bool) {
	if !m.config.Metrics.Enabled {
		return
	}
	...
	m.MCPBotInteractions.WithLabelValues(botType, interactionType, tenantID, status).Inc()
}

// RecordMCPChannelActivity registra atividade de canal
func (m *Metrics) RecordMCPChannelActivity(channel, activityType, tenantID string) {
	if !m.config.Metrics.Enabled {
		return
	}

	m.MCPChannelActivity.WithLabelValues(channel, activityType, tenantID).Inc()
}